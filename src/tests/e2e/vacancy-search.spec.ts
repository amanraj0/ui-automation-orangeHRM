...
test(
  ...
  async ({ page }) => {
    ...
    await test.step("Search for Vacancy", async () => {
      await vacancyPage.searchWithVacancyname(VacancyDetails.name);

      const actualVacancyDetails =
        await vacancyPage.getVacancySearchResultString();

      expect(
        actualVacancyDetails,
        "User should be able to search for Vacancy based on name",
      ).toContain(`${VacancyDetails.name}`);
    });
  },
);
